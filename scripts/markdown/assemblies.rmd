...
in_data = subset(in_data, Include=="Y")
...
# Order the data by number of scaffolds
...
# Order the data by scaffold N50
...
n50_p = ggplot(in_data, aes(x=label, y=Scaffold.N50, color=Annotation.file)) +
  ...
  geom_point(size=4) +
  scale_y_continuous(expand=c(0,0), limits=c(0,max(in_data$Scaffold.N50, na.rm=T) + 7500000)) +
  ylab("Scaffold N50") +
  xlab(paste("")) +
  scale_color_manual(name="Annotated", values=c("Y"=corecol(pal="wilke", numcol=1, offset=1), "N"=corecol(pal="wilke", numcol=1))) +
  bartheme() +
