...
missing_p = ggplot(windows, aes(x=scaffold, y=seqs.above.missing, fill=scaffold)) +
  #geom_quasirandom(size=2, width=0.25, alpha=0.1, color="#666666") +
  #geom_violin(alpha=0.5) +
  geom_boxplot(outlier.shape=NA, alpha=0.5, width=0.5, color="#000000") +
  #geom_point(data=aln_means, aes(x=label, y=avg.length, group=dataset), size=2, color="red", position=dodge, show.legend=FALSE) +
  #geom_point(data=aln_means, aes(x=label, y=median.length, group=dataset), size=2, color="blue", position=dodge, show.legend=FALSE) +
...
## Masked seqs per window

```{r mask-seqs, out.width="50%", fig.align = "center", warning=FALSE, fig.height=4}

masked_p = ggplot(windows, aes(x=scaffold, y=seqs.above.repeat, fill=scaffold)) +
  #geom_quasirandom(size=2, width=0.25, alpha=0.1, color="#666666") +
  #geom_violin(alpha=0.5) +
  geom_boxplot(outlier.shape=NA, alpha=0.5, width=0.5, color="#000000") +
  #geom_point(data=aln_means, aes(x=label, y=avg.length, group=dataset), size=2, color="red", position=dodge, show.legend=FALSE) +
